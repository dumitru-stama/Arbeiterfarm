...
| **E**levation of Privilege | **Sandbox Escape** | Malware exploits kernel to take over host. | **Fail-Closed Sandbox**: `--unshare-all` (PID/IPC/UTS/net/cgroup isolation), `--tmpfs /` empty root, `--clearenv`, `--die-with-parent`, `--cap-drop ALL`. Fail-closed: RE/VT/Ghidra tools are not registered when `af-executor` is missing (unless `AF_ALLOW_UNSANDBOXED=1` is explicitly set). All tool executors (including VT) run in bwrap sandbox via OOP executor. |
...
| ~~**Sandbox Silent Bypass**~~ | **RESOLVED**: RE/VT/Ghidra tools are no longer registered when `af-executor` is missing. Previously, missing executor silently fell back to InProcess (no bwrap). Now requires explicit `AF_ALLOW_UNSANDBOXED=1` env var for InProcess mode. | `arbeiterfarm/src/main.rs` |
